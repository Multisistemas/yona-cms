...
<div id="profiler">
    <div class="bar clearfix" onclick="profilerToogle();">
        ...
        <ul class="info">
            <li>Total SQL queries: <?= $this->profiler->getNumberTotalStatements() ?></li>
            <?php $seconds = round($profiler->getTotalElapsedSeconds(), 5) ?>
            <li>Total SQL elapsed seconds: <?= sprintf(($seconds * 1000), '%01.2f') ?> ms</li>
        ...
        <?php if ($this->profiler->getProfiles()) { ?>
            ...
            <div class="profile">
                <dl>
                    ...
                    <dd><?= sprintf(($seconds * 1000), '%01.2f') ?> ms</dd>
                </dl>
            </div>
            <?php } ?>
        <?php } ?>
